...
export const ChiefRegister = ({ history }) => {
  ...
  const onChange = (ev) => {
    ...
    if (ev.target.name === "username")
      setUsernameError(validUsername(ev.target.value));
    ...
    // Make sure email is valid
      ...
      setFCEmailError(validFCEmail(ev.target.value));
  ...
  const onSubmit = async (ev) => {
    ...
    if (res.ok) {
      // Notify users
      window.alert(
        `${state.username} successfully registered.  You will now need to log in.`
      );
    ...
    <PageContainer>
      ...
      <OwnerRegisterContainer>
        ...
        <div id="message" style={{ display: "none" }}>
          ...
          <p id="letter" style={invalid}>
            <span id="letterSpan">{letterText}</span>A <b>lowercase</b> letter
          </p>
          <p id="capital" style={invalid}>
            <span id="capitalSpan">{capitalText}</span>A{" "}
            <b>capital (uppercase)</b> letter
          </p>
          <p id="number" style={invalid}>
            <span id="numberSpan">{numberText}</span>A <b>number</b>
          </p>
          <p id="char" style={invalid}>
            <span id="charSpan">{charText}</span>A <b>special character</b>
          </p>
          <p id="length" style={invalid}>
            <span id="lengthSpan">{lengthText}</span>Minimum <b>8 characters</b>
          </p>
        </div>
        <select
          id="department"
        ...
        >
          <option default value="default" disabled>
            Select Department
          </option>
