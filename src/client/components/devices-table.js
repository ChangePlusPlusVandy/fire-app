...
export const DevicesTable = (props) => {
  ...
  useEffect(() => {
    fetchFirechief(props.match.params.username).then(() => {});
  }, []);
  ...
  const fetchDevices = async () => {
    let devices = [];
    await Promise.all(
      firechief.controllable_firezones.map(async (firezone) => {
  ...
  const activateFirezone1 = async (ev) => {
    ev.preventDefault();
    const zone1Devices = state.devices.filter(
      (device) => device.firezone === 1
    );
    const firezone = 1;
  ...
  if (isLoaded) {
    const deviceList = state.devices.map((device, index) => {
      return <SprinklerHub key={index} device={device} />;
    });
    ...
    return (
      <PageContainer>
        ...
        <p style={{ width: "100%", marginLeft: "50px" }}>
          Number of Devices={state.devices.length}
        </p>
        <ChiefButtonsContainer>
          <FreeButton
