...
module.exports = (app) => {
  ...
  app.post("/v1/device", async (req, res) => {
    ...
    try {
      ...
      await Promise.all(
        person_info.devices.map(async (item, i) => {
          let device = {
          ...
          device["firezone"] = closestFirezone(
            item,
            firezones,
            maxDistance
          ).number;
  ...
  app.put("/v1/devices/startzone/:firezone", async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .send({
          error: `Error starting Firezone ${req.params.firezone}: ${err.message}`,
        });
    }
  });
  ...
  app.put("/v1/devices/stopzone/:firezone", async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .send({
          error: `Error stopping Firezone ${req.params.firezone}: ${err.message}`,
        });
    }
  });
  ...
  app.delete("/v1/devices/:id", async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .send({
          error: `Error deleting device ${req.params.id}: ${err.message}`,
        });
    }
  });
};
