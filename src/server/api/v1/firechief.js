...
module.exports = (app) => {
  ...
  app.post("/v1/firechief", async (req, res) => {
    ...
    if (pwdErr) {
      console.log(
        `Firechief.create password validation failure: ${pwdErr.error}`
      );
  ...
  app.head("/v1/firechief/:username", async (req, res) => {
    ...
    if (!firechief)
      res
        .status(404)
        .send({ error: `unknown firechief: ${req.params.username}` });
  ...
  app.delete("/v1/firechief/:username", async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .send({
          error: `Error deleting firechief ${req.params.username}: ${err.message}`,
        });
    }
  });
  ...
  app.put("/v1/firechief/authorize/:username", async (req, res) => {
    ...
    try {
      const chief = await app.Models.Firechief.findOne({
      ...
      res
        .status(400)
        .send({
          error: `Error changing authorization for firechief ${req.params.username}: ${err.message}`,
        });
    }
  });
};
